...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Automatically start recognition on page load
  recognition.start();
  ...
  recognition.addEventListener("result", (event) => {
    ...
    // Map recognized words to checkbox IDs
    const needsMap = {
      "visual disabilities": "visualDisabilities",
      "dyslexia": "dyslexiaLearningDisabilities",
      "learning disabilities": "dyslexiaLearningDisabilities",
      "adhd": "adhd",
      "photosensitivity": "photosensitivity",
      "neurological": "neurologicalCognitive",
      "cognitive": "neurologicalCognitive",
      "motion sensitivity": "motionSensitivity",
      "other": "other"
    };

    // Check or uncheck checkboxes based on recognized words
    let matched = false;
    for (const [key, checkboxId] of Object.entries(needsMap)) {
    ...
    if (!matched) {
      voiceStatus.textContent += " (No matching needs found)";
    }
  });
});
